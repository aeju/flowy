...
Unity 기반 MES(제조실행시스템) 미니 시뮬레이터로, 여러 공정(W1~W4)을 거치며 제품이 처리되는 생산 라인을 단순화해 구현했습니다. 공정 상태 관리, 실시간 시각화, 조작, 핵심 지표 집계를 다룹니다. Unity로 최초 구현한 뒤 C#/WPF 데스크톱 앱으로 포팅했으며, 현재는 WPF 버전이 메인입니다.
...
- **WPF 버전**: 제조 스택(C#/WPF)으로 포팅 완료 (현재 메인)
...
가장 큰 목표는 화려한 그래픽이 아니라, **MES의 핵심 개념(실시간 상태 관리, 이벤트 기반 아키텍처, 지표 집계, SQL 기반 분석)을 실제로 동작하는 코드로 증명하는 것**이었습니다.
