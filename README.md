...
![병목 분석 팝업](docs/images/wpf-bottleneck-report.png)
...
| 가동률 | 현재 순간 Running 설비 비율 | Running 총 체류시간 / 전체 관측시간 (설비별 시간가중, 세션 도입 이후 전체 이력 기반) |

누적 가동률은 화면 상단에도 상시 표시해, 실시간과 누적을 한 화면에서 대조할 수 있습니다. 매 tick DB 쿼리 부담을 줄이기 위해 5 tick마다 갱신됩니다.
...
| 누적 가동률 | 구현 완료 | 설비별 Running 총 체류시간 / 전체 관측시간 × 100 (시간가중, DB 집계). 화면 상단 상시 표시 + 병목 분석 리포트 상세 |
