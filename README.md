...
여러 공정(W1~W4)을 거치며 제품이 처리되는 생산 라인을 단순화해 구현한 MES(제조실행시스템) 미니 시뮬레이터입니다. C#/WPF 데스크톱 앱이 메인이며, 공정 상태 관리, 실시간 시각화, 조작, 핵심 지표 집계를 다룹니다. 설비 이벤트는 SQLite에 적재해 SQL로 병목과 가동률을 분석하고, 분석 결과는 읽기 전용 Web API로도 제공합니다. Unity로 처음 구현한 뒤 WPF로 포팅했습니다.

## 바로 실행해보기

- **WPF (Windows)**: [Releases](https://github.com/aeju/flowy/releases)에서 `Flowy-v1.0-win-x64.zip`을 받아 압축 해제 후 `Flowy.Wpf.exe` 실행. .NET 런타임 설치 불필요(self-contained)
- **Unity (웹)**: [웹 시연 링크](https://aeju.github.io/flowy/)에서 바로 확인
- **Web API**: 별도 실행 파일 없음. 소스 빌드 후 실행 → [소스에서 실행](#소스에서-실행) 참고 (Swagger 화면은 [실행 화면](#실행-화면)에서 확인)
...
생산 라인의 각 공정(W1~W4)이 대기(Idle) → 가동(Running) → 이상(Error) 상태를 오가며, 상태 변화가 목록·상단 KPI·이력 그리드·하단 Alert에 실시간으로 반영됩니다. 사용자는 버튼으로 제품을 투입하거나, 라인 속도를 조절하거나, 설비를 강제로 정지/재가동시킬 수 있습니다.

그래픽보다는 MES의 핵심 개념(실시간 상태 관리, 이벤트 기반 아키텍처, 지표 집계, SQL 기반 분석)을 실제로 동작하는 코드로 구현하는 데 초점을 두었습니다.
...
![WPF 실행 화면](docs/images/wpf-main-screen-3.png)
...
아래는 **WPF 버전(현재 메인)** 기준입니다. Unity 버전은 중앙 3D 큐브 뷰로 공정 상태(가동=초록 / 대기=노랑 / 이상·정지=빨강)를 시각화하며, 병목 판별·자동 투입·이력 그리드 등은 WPF에서 추가된 기능입니다.

**좌측 — 제어**
- **제품 투입**: W1에 제품을 하나씩 수동 투입 (몰아넣으면 Push 과부하 시연)
- **자동 투입**(체크박스): 켜면 라인 소화 속도에 맞춰 자동 공급, 병목을 안정적으로 관측
- **설비 정지 / 재가동**: 공정을 강제로 멈추거나 다시 가동
- **가속 / 감속**: 시뮬레이션 tick 속도 조절
- **병목 분석**: 실시간 병목(큐 기반)과 누적 병목(DB Error 체류시간), 설비별 누적 가동률을 함께 산출. 결과를 팝업으로 즉시 표시하고 리포트 파일(`bottleneck_report_시각.txt`)로도 저장

**우측 — 모니터링**
- **공정 목록**: 각 공정의 이름·사이클타임(C/T)·현재 상태(색상 구분). 앞에 대기가 쌓이면 대기 수 표시(병목 신호)
- **이력 그리드**: 상태 전이 이력(설비·상태·시각)을 최신순으로 표시

**상단 — 지표**
- **KPI**: 실시간 가동률(스냅샷)과 누적 가동률(설비별 시간가중)을 나란히 표시. UPH 계산 표시. 양품률은 미표시("--")
- **우상단**: 현재 시각, 시뮬레이션 가동 시간·완성 개수

**하단 — 알림**
- **Alert**: 이상(Error)·정지 공정 안내, 병목 분석 결과 표시
  
---

## 아키텍처

3개 레이어로 분리했습니다. 아래는 Unity 원본 구조이며, WPF 이관 후 폴더 구조는 [Unity → WPF 이관](#unity--wpf-이관) 섹션 참조.
...
### 6. SQL 집계와 실시간/누적 지표 분리

SQLite에 상태 전이 이력을 적재하는 것에서 나아가, 집계 쿼리로 실제 분석 지표를 산출하도록 확장했습니다. 초기에는 이력을 저장만 하고 병목 판정은 메모리 큐로만 처리해, 데이터베이스가 사실상 write-only였습니다. 이를 SQL 집계 분석기(EventAnalyzer)를 도입해 해소했습니다.
...
이벤트 로그는 상태 전이 시점만 기록하므로, 특정 상태에 얼마나 머물렀는지를 구하려면 다음 이벤트 시각을 당겨와 차분해야 합니다. `LEAD(Timestamp) OVER (PARTITION BY MachineName, SessionId ORDER BY Timestamp)`로 설비·세션 단위로 다음 시각을 붙이고, `julianday()`로 두 시각의 초 단위 차이를 산출했습니다.

**세션 ID 도입 (세션 경계 오염 방지)**

앱 실행마다 GUID로 세션을 식별해, 여러 실행 이력이 한 테이블에 섞여도 세션 경계를 넘지 않고 집계됩니다. 세션 ID 도입 전 기존 데이터는 legacy로 자동 보존하고, 세션 경계가 불분명해 분석에서 제외합니다. 세션 ID로 GUID를 택한 이유는 Timestamp 컬럼과 정보가 중복되지 않도록 역할을 완전히 분리하기 위함이며, 시각 정보가 필요하면 `MIN(Timestamp)`로 조회합니다.

**실시간/누적 두 관점 병행**

병목과 가동률 모두 실시간(스냅샷)과 누적(시간가중) 두 관점으로 분리해 병행 표시합니다. 정의가 다른 두 지표를 같은 이름으로 두면 혼동되기에, 이름·표시 위치를 명확히 나눴습니다.

| 지표 | 실시간 | 누적 |
|---|---|---|
| 병목 | 큐 기반 (지금 어디 막혔나) | DB 기반 Error 체류시간 (전체 구간에서 어느 설비가 총 정지시간 최다) |
| 가동률 | 현재 순간 Running 설비 비율 | Running 총 체류시간 / 전체 관측시간 (설비별 시간가중) |

실측 결과 세 지표가 서로 다른 문제를 잡아냅니다. 예를 들어 실시간 병목이 W1(사이클타임 3초의 앞단이 자동 투입으로 과부하)을 가리키는 상황에서, 누적 Error 병목도 W1(Error 81회, 503초)이 최다였지만, 누적 가동률은 W3·W4가 45%·56%로 낮아 앞 공정 지연으로 굶고 있음(starvation)을 별개로 드러냈습니다. 하나의 지표만 봤으면 놓쳤을 문제를 서로 다른 관점이 잡아준 결과입니다.
...
| 지표 | 상태 | 설명 |
|---|---|---|
| 실시간 가동률 | 구현 완료 | 현재 순간 Running 상태인 공정 수 / 전체 공정 수 × 100 (스냅샷) |
| 누적 가동률 | 구현 완료 | 설비별 Running 총 체류시간 / 전체 관측시간 × 100 (시간가중, DB 집계) |
...
- SQL 집계 세션 경계 오염: 앱을 여러 번 실행하며 쌓인 이력이 한 테이블에 섞여, `LEAD`로 다음 이벤트를 당겨오면 다른 실행의 이벤트가 붙어 수십 시간짜리 가짜 체류시간이 계산되는 문제 -> 세션 ID(GUID) 도입 및 `PARTITION BY MachineName, SessionId`로 세션 내에서만 차분

---

## Unity → WPF 이관

제조 현장에서 널리 쓰이는 스택(C#/WPF)에 맞춰, Unity 버전을 WPF 데스크톱 앱으로 포팅했습니다. 게임 엔진이 아닌 실제 제조 IT의 UI 프레임워크로 옮기는 것이 목표였습니다.
...
- ✅ **상태 색상 표시** — 공정 상태를 색으로 구분 (IValueConverter로 상태→색 변환, MVVM 유지)
- ✅ **자동 투입(일정 속도 공급)** — 체크박스로 on/off, 라인 소화 속도(3tick)에 맞춰 W1에 자동 공급. 수동 투입도 유지
- ✅ **이벤트 세션 분리** — 앱 실행마다 GUID 세션 ID 발급, 여러 실행 이력이 섞여도 세션 경계 유지. 기존 데이터는 legacy로 보존
- ✅ **SQL 집계 분석기 도입** — EventAnalyzer(읽기 전용) 신설, LEAD 윈도우 함수로 상태별 체류시간 산출. Repository와 책임 분리(CQRS 축소판)
