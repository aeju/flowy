...
```
Flowy | 가동률 0.0% | UPH -- | 양품률 -- |   <- 상단 KPI (OEE 3요소)
...
- **상단 KPI**: OEE 3요소 중 가동률만 계산해 표시. UPH·양품률은 산출·불량 데이터가 없어 "--"로 표시
...
| UPH (성능) | 미표시 | 산출 개수 / 시간. 계산하려면 "사이클 타임(공정당 소요 시간)" 가정이 필요. 값은 임의지만 규칙은 실제 모델이라, 도입 시 결정론적으로 도출 가능 |
| 양품률 (Quality) | 미표시 | 양품 / 전체. 계산하려면 불량이 발생하는 메커니즘이 필요. 그것 없이 불량률을 넣으면 근거 없는 값을 지어내는 것이라, 도입하지 않음 |
| OEE | 미표시 | 가동률 × 성능 × 양품률. 세 요소가 모두 있어야 산출되며, 성능·양품률이 비어 있어 표시하지 않음 |

OEE 3요소 중 계산 가능한 것은 가동률뿐입니다.
UHP는 사이클 타임 가정을 도입하면 계산 가능하지만(구조적 가정), 양품률은 불량 발생 근거가 없어 값을 지어내지 않는 한 계산할 수 없습니다. 근거로 도출되는 값과 지어내야 하는 값을 구분해, 계산 가능한 가동률만 표시하고 나머지는 "--"로 남겼습니다.
...
- ✅ **SQLite 연동** — 시뮬레이션 이벤트를 SQLite+Dapper로 적재. 시작 시 이력 복원. OEE 3요소 KPI 표시(계산 가능한 가동률만, UPH·양품률은 "--")
